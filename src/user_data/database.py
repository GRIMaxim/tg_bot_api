...
class UserData(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        __type_pos=BigInteger, nullable=True, unique=True,
    )
    ...
    start_subs_at: Mapped[datetime] = mapped_column(
        __type_pos=TIMESTAMP(timezone=True), nullable=True,
    )
    end_subs_at: Mapped[datetime] = mapped_column(
        __type_pos=TIMESTAMP(timezone=True), nullable=True,
    )
    is_trial_used: Mapped[bool] = mapped_column(default=False)
    online_search_active: Mapped[bool] = mapped_column(default=False)

    rs_user_chat: Mapped[list[UserChat]] = relationship(lazy="selectin")

    chats: AssociationProxy[list[str]] = association_proxy("rs_user_chat", "chat_name")
